/*
 ...
 -------------------------------------------------------------- 
   Written by Parisa Babaheidarian, Basis Decomposition project: 
           Functions and Jacobians for the PCA transform
           Change number 5 in the code to the number of bases
...
public:
	 ...
	 double I0;
	Essentials(double x[], double B[5][100], double Weight [10][100]) {
		I0 = 2 * pow(10, 5);
...
vector<vector<double> > Jacd(vector<vector<double> >  alpha, vector<vector<double> > Wbrute) {
  ...
  double * Grad(
		double bin1, double bin2, double bin3, double bin4, double bin5, double bin6, double bin7, double bin8, double bin9, double bin10)
	{
		...
		vector<vector<double> > temp1(10, vector<double>(5, 0));
	...
	vector<vector<double> > result(1,vector<double> (5,0)); 

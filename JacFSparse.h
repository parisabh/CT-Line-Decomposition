...
public:
	 ...
	 double I0;
	Essentials(double x[], double B[12][100], double Weight [10][100]) {
		I0 = 2 * pow(10, 5);
		vector<double> tmp(12, 0);
		alpha.resize(1, tmp);
		for (int i = 0; i < 12; i++) {
			alpha[0][i]=x[i];
		}
		vector<double> temp(100, 0);
		Wbrute.resize(12,temp);
		for (int i = 0; i < 12; i++) {
...
vector<vector<double> > Jacd(vector<vector<double> >  alpha, vector<vector<double> > Wbrute) {
  ...
  double * Grad(
		double bin1, double bin2, double bin3, double bin4, double bin5, double bin6, double bin7, double bin8, double bin9, double bin10)
	{
		...
		vector<vector<double> > temp1(10, vector<double>(12, 0));
        ...
        //new line added for sparsity penalty with lambda=0.001
         vector<vector<double> >a(1,vector<double>(12, 0.001));
	...
	vector<vector<double> > result(1,vector<double> (12,0)); 
    result= Multi(g,temp1);
    static double output[12];
	for (int i = 0; i < 12; i++) {
		output[i] = result[0][i]+a[0][i];
	}
	return output;
	}
		...
		double bin1, double bin2, double bin3, double bin4, double bin5, double bin6, double bin7, double bin8, double bin9, double bin10)
	{
        ...
        // a is for sparsity penalty with lambda=0.001
        vector<vector<double> > a(1,vector<double>(12,0.001));
    ...
    temp5=MultiT(a,alpha);
	return 0.5*temp4[0][0]+temp5[0][0];
